...
handle["runInWolfram", json_]:=Module[{range, uri, src, ast, codeRange, code},
	...
	ast = CodeParse[src];
	codeRange = Echo@FirstCase[ast,
		CallNode[___,<|Source->{{l1_,c1_},{l2_,c2_}},___|>]/;(l1<=range["start"]["line"]+1&&l2>=range["end"]["line"]+1)&&(c1<=range["start"]["character"]):>{{l1,c1},{l2,c2}},{{range["start"]["line"]+1,range["start"]["character"]+1},{range["end"]["line"]+1,range["end"]["character"]+1}},
		{2}];

	Print["runInWolfram"];
	code = StringTakeLineCharacters[src, codeRange];

	result = Echo@RemoteEvaluate[EKernel, 
		ToExpression[
			"EvaluationData["<>code<>"]", 
			InputForm, 
		    Unevaluated]
...
handle["codeLens/resolve", json_]:=Module[{},
	sendResponse[<|"id" -> json["id"], "result"-><|"command"-><|"title"->"Wolfram Language", "command"->"wolframLanguage.moveCursor"|>|>|>];
];
...
StringTakeLineCharacters[string_,{{l1_,c1_},{l2_,c2_}}]:=StringTake[
	string,
	{
		StringPosition["\n"<>string, "\n"][[l1,1]]+c1-1, 
		StringPosition["\n"<>string, "\n"][[l2,1]]+c2-2
...
handle["textDocument/documentSymbol", json_]:=Module[{uri, src, tree, symbols, functions, result, response, kind},
		(
			...
			symbols = Cases[ast,CallNode[LeafNode[Symbol, ("Set"|"SetDelayed"),_],
			{(
				CallNode[LeafNode[Symbol,name__,<|Source->_|>],{},<|Source->_|>] |
				LeafNode[Symbol, name__, _]
			),___},___,<|Source->s_,"Definitions"->{_}|>]:>{name, s},{4}]
			...
			result = Table[
				...
				<|
					...
					"location"-><|
						"uri"->uri,
						"range"-><|
							"start"-><|"line"->s[[2,1,1]]+1,"character"->s[[2,1,2]]+1|>,
							"end"-><|"line"->s[[2,2,1]]+1,"character"->s[[2,2,2]]+1|>
							|>
						|>
				|>, 0.05, 
				<|
