...
async function startParserService(): Promise<void> {
    outputChannel.appendLine("Starting Wolfram parser services...");
...
async function ensureTemporaryDirectory(): Promise<void> {
    ...
    temporaryDir = path.join(base, 'wlsp-temp');
    try {
        await fsPromises.mkdir(temporaryDir, { recursive: true });
    } catch {
        // ignore mkdir failures; later callers will handle lack of temp dir.
    }
}

async function analyzeDocumentStructure(document: vscode.TextDocument | undefined): Promise<void> {
    if (!parserService || !document || document.languageId !== 'wolfram') {
        return;
    }

    try {
        const analysis = await parserService.analyzeDocument(document);
        const key = decodeURIComponent(document.uri.fsPath.toString());
        movePositions[key] = { locations: analysis.ranges };

        if (vscode.window.activeTextEditor?.document.uri.toString() === document.uri.toString()) {
            cursorLocations = analysis.ranges.map((range) => ({
                start: { line: range.start.line, character: range.start.character },
                end: { line: range.end.line, character: range.end.character },
            }));
        }
    } catch (error) {
        outputChannel.appendLine(`Parser analysis failed: ${error}`);
    }
}
...
export async function onParserReady(): Promise<void> {
    outputChannel.appendLine("Wolfram parser ready.");
    await ensureTemporaryDirectory();
    await Promise.all(
        vscode.workspace.textDocuments
            .filter((doc) => doc.languageId === 'wolfram')
            .map((doc) => analyzeDocumentStructure(doc)),
    );
}
...
function handleWorkspaceFiles(): void {
    ...
    workspaceFolders.forEach((folder: WorkspaceFolder) => {
        if (wolframKernelClient?.state !== State.Running) {
            outputChannel.appendLine("Wolfram Kernel not running, cannot send workspace folders.");
            return;
        }
        ...
        try {
            ...
            outputChannel.appendLine(`Failed to notify kernel about workspace folder: ${error}`);
        }
    });
}
...
async function didChangeTextDocument(event: vscode.TextDocumentChangeEvent): Promise<void> {
    return new Promise((resolve) => {
        ...
        analyzeDocumentStructure(event.document);
...
function handleDocumentWithoutFolder(document: vscode.TextDocument): void {
    if (document.languageId === 'wolfram' && !clients.has("default")) {
        totalClients++;
        clients.set("default", [undefined, wolframKernelClient]);
    }
...
function handleDocumentWithFolder(document: vscode.TextDocument, folder: vscode.WorkspaceFolder): void {
    if (!clients.has(folder.uri.toString()) && document.languageId === "wolfram") {
        totalClients++;
        clients.set(folder.uri.toString(), [undefined, wolframKernelClient]);
    }
}
...
function didSaveTextDocument(event: vscode.TextDocument): void {
    ...
    analyzeDocumentStructure(event);
}
