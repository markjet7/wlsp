...
import {treeDataProvider, wolframKernelClient} from './clients'
...
export class workspaceSymbolProvider implements vscode.TreeDataProvider<TreeItem> {
    ...
    async getBuiltins() {
        const completionsPath = path.join(__dirname, '..', '..', 'wolfram', 'completions.json');
        fs.readFile(completionsPath, 'utf8', (err: any, raw: string) => {
            if (err) {
                console.error(`Unable to load Wolfram completions: ${err}`);
                return;
            }
            ...
            const builtinsymbols = completions.map((symbol: any) => {
                const item = new TreeItem(symbol.label ?? '');
                item.tooltip = symbol.detail ?? symbol.documentation ?? symbol.label;
                item.command = {
                    command: 'wolfram.stringHelp',
                    arguments: [symbol.label],
                    title: 'Open',
                };
                return item;
            });
            ...
            if (builtins.children?.length === 0) {
                builtins = new TreeItem(
                    'Builtins',
                    letters.map((letter: string) => {
                        return new TreeItem(
                            letter,
                            builtinsymbols.filter(
                                (item: TreeItem) =>
                                    typeof item.label === 'string' &&
                                    item.label.toString().startsWith(letter),
                            ),
                        );
                    }),
                );
            ...
            doc.command = {
                ...
                arguments: ['https://reference.wolfram.com/language/'],
                title: 'Open',
            };

            let functionRepository: TreeItem = new TreeItem('Function Repository');
            functionRepository.command = {
                command: 'wolfram.wolframHelp',
                arguments: ['https://resources.wolframcloud.com/FunctionRepository/'],
                title: 'Open',
            };
